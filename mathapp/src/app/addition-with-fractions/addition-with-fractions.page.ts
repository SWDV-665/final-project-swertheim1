...
@Component({
  ...
  templateUrl: './addition-with-fractions.page.html',
  styleUrls: ['./addition-with-fractions.page.scss'],
})
export class AdditionWithFractionsPage implements OnInit {
    ...
    randomNumerator1: number = 0;
    randomNumerator2: number = 0;
    randomDenominator1: number = 0;
    randomDenominator2: number = 0;
    ...
    constructor(private hapticsService: HapticsService, private route: ActivatedRoute, private router: Router, private toastController: ToastController, private resultsDataService: ResultsDataService) {
        console.log('Addition with fractions constructor accessed');
    }
    ...
    generateProblem(): void {
        ...
        this.randomNumerator1 = this.generateRandomWholeNumbers(this.MIN_ONE, this.MAX_ONE);
        this.randomNumerator2 = this.generateRandomWholeNumbers(this.MIN_TWO, this.MAX_TWO);
        this.randomDenominator1 = this.generateRandomWholeNumbers(this.MIN_ONE, this.MAX_ONE);
        this.randomDenominator2 = this.generateRandomWholeNumbers(this.MIN_TWO, this.MAX_TWO);
        
        this.generateExpression(this.randomNumerator1, this.randomDenominator1, this.randomNumerator2, this.randomDenominator2);
        console.log(`'Random numbers: ', ${this.randomNumerator1}, '/' ${this.randomDenominator1}, 'Expression: ', ${this.expression}, 'Answer: ',${this.randomNumerator2}, '/' ${this.randomDenominator2}`)
    }

    generateExpression(randomNumerator1: number, randomDenominator1: number, randomNumerator2: number,randomDenominator2: number) {
        console.log('generate an expression function accessed')
        this.answerList = [];

        if (this.numberOfQuestionsAsked < this.totalQuestionsToAsk) {
            this.answer = (fractional(this.randomNumerator1, this.randomDenominator1)).add(fractional(this.randomNumerator2, this.randomDenominator2)).toString();
    ...
    calculateAnswer(numerator1: number, denominator1: number, numerator2: number, denominator2: number) {
      var fraction: any = (fraction(numerator1, denominator1)).add(fraction(numerator2, denominator2));
